import"./assets/styles-cd1a6381.js";const c=document.querySelector(".scrol-list"),p="BASKET",s=document.querySelector(".empty-yellow-cart"),r=document.querySelector(".scrollbar-container"),a=document.querySelector(".order-container"),i=document.querySelector(".quantity-in-cart"),d=document.querySelector(".quantity-in-cart-header"),g=document.querySelector(".cart-sum-span");let t=JSON.parse(localStorage.getItem(`${p}`));console.log(t);function y(e){if(!e||e.length===0){s.style.display="block",r.style.display="none",a.style.display="none";return}const o=t.reduce((n,m)=>n+m.price,0).toFixed(2);g.textContent=`$${o}`;const l=e.map(n=>`<li class="scroll-item">
            ...
            </li>`).join("");c.innerHTML=l,i.textContent=e.length,d.textContent=e.length}y(t);const S=document.querySelector(".cart-button");S.addEventListener("click",h);function h(e){s.style.display="block",r.style.display="none",a.style.display="none",i.textContent=0,d.textContent=0,localStorage.removeItem("BASKET"),c.innerHTML=""}c.addEventListener("click",C);function C(e){if(!e.target.classList.contains("scroll-top-button"))return;const u=e.target.id,o=t.findIndex(l=>l._id===u);o!==-1&&(t.splice(o,1),console.log(t),y(t),localStorage.setItem(`${p}`,JSON.stringify(t))),t.length||(i.textContent=t.length,d.textContent=t.length,s.style.display="block",r.style.display="none",a.style.display="none")}
